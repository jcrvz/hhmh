function [bestAgent,bestFitness,details] = DSOA(ObjectiveFunction,PopInitRange)
%
...
% ------------------------------------------------------------------------
    ...
    function SpiralDynamics()
        for AgentId = 1 : PopulationSize 
            ...
            CurrentPopulation(:,AgentId) = Radius*...
                RotationMatrix*CurrentPopulation(:,AgentId) - ...
                ((Radius*RotationMatrix - IdentityMatrix)*bestAgent);
        end
    end
