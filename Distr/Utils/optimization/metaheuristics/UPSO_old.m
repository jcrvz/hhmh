...
% Advised by Rodrigo Correa, Ph.D. and Iv�n Amaya, Ph.D(c).
%
% CEMOS Research Group - Grupo de Investigaci�n CEMOS
...
function [Pg,fPg,details, varargout] = UPSO_old(fObj,bnd,parameters,oldX)
...
%         targetIndex = X(i,:) < bnd(:,1)' | X(i,:) > bnd(:,2)';
%         X(i,targetIndex)   = bli(targetIndex) + rand(1,sum(targetIndex)).*bdif(targetIndex);
        for d = 1 : Nd
            if X(i,d) < bnd(d,1) || X(i,d) > bnd(d,2),
                X(i,d)   = bli(d) + rand*bdif(d);
            end
        end
...
details = struct('time',t,'fevs',(Na + 1)*steps,'steps',steps,...
